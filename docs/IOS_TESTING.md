...
The shared UI is a 10-foot television layout. `apps/ui/src/theme/mobile.css` (imported last from `apply.ts`) narrows rails, raises tap targets, applies `safe-area-inset-*`, keeps the ribbon on screen, and hides the D-pad remote lesson. The native WebView adds `viewport-fit=cover` so those insets resolve.
...
- Browse, search, select episodes, open/back out of details and change profiles with touch. Check small portrait and landscape screens, safe areas and readable controls. Enable Reduce Motion in iOS and in TVM.
