...
/**
 * The live endpoint for one channel.
 *
 * `.ts` — raw MPEG-TS — is the canonical Xtream live endpoint and the one
 * panels reliably serve. `.m3u8` is generated by an HLS module that many panels
 * ship without licensing or configuring properly: the manifest returns 200 and
 * looks perfect, and then the CDN in front of it answers every segment with
 * 403. That failure is invisible from the manifest alone and reaches the viewer
 * as a bare "this stream could not start".
 *
 * Verified against a live subscription: `.m3u8` manifest 200 with every segment
 * 403 on two different channels, while `.ts` returned real MPEG-TS on both.
 * `GET /api/live/diagnose/:id` reports exactly this comparison.
 *
 * Both phones and the desktop player read MPEG-TS natively, so this costs
 * nothing. Set TVM_XTREAM_HLS=1 for a panel whose HLS genuinely works and which
 * does not serve `.ts`.
 */
export function xtreamStreamUrl(account: XtreamAccount, streamId: string): string {
  const base = `${account.host}/live/${encodeURIComponent(account.username)}/${encodeURIComponent(account.password)}/${encodeURIComponent(streamId)}`;
  return process.env['TVM_XTREAM_HLS'] === '1' ? `${base}.m3u8` : `${base}.ts`;
}
