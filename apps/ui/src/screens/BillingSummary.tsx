...
import { applyPlanClass, cancelPlan, fetchBilling, formatBillingMoney, type BillingStatus } from '../data/plan';
...
export function BillingSummary({ onChange }: { onChange: () => void }): React.JSX.Element {
  ...
  return <section className="billing-panel">
    ...
    {billing && <>
      ...
      <p className="billing-fineprint">TVM plans provide app features. Provider subscriptions and rights to watch content are separate. Colourcast is a one-time visual pack.</p>
      ...
      {confirm && <p>Cancellation takes effect now. Colourcast stays unlocked. No refund is due because no money was taken.</p>}
